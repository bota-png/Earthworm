<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <title>Virtual Earthworm Dissection Lab</title>
  <style>
    body {
      font-family: Arial, sans-serif;
      background-color: #f9f9f9;
      margin: 0;
      padding: 0;
    }
    header {
      background-color: #4CAF50;
      color: white;
      padding: 1em;
      text-align: center;
    }
    .container {
      padding: 2em;
      max-width: 900px;
      margin: auto;
    }
    section {
      background: white;
      padding: 1.5em;
      border-radius: 10px;
      box-shadow: 0 2px 5px rgba(0,0,0,0.1);
      margin-bottom: 2em;
    }
    h2 {
      color: #333;
    }
    iframe {
      width: 100%;
      height: 400px;
      border: none;
    }
    button {
      padding: 10px 20px;
      background-color: #4CAF50;
      color: white;
      border: none;
      border-radius: 5px;
      cursor: pointer;
      margin-top: 10px;
    }
    textarea {
      width: 100%;
      padding: 10px;
      margin-top: 10px;
      border-radius: 5px;
      border: 1px solid #ccc;
    }
  </style>
</head>
<body>
  <header>
    <h1>Virtual Earthworm Dissection Lab</h1>
  </header>

  <div class="container">
    <section>
      <h2>🔬 Context</h2>
      <p>In this virtual lab, you will perform a dissection of an earthworm. You'll learn about its internal organs and functions. Follow each step carefully and complete the tasks.</p>
    </section>

  <section>
      <h2>📋 Lab Protocol</h2>
      <ol>
        <li>Watch the dissection video below.</li>
        <li>Explore the labeled internal parts of the earthworm.</li>
        <li>Take notes on what you observe.</li>
        <li>Answer the questions in the worksheet provided.</li>
      </ol>
    </section>

  <section>
      <h2>🎥 Dissection Video</h2>
      <iframe src="https://www.youtube.com/embed/aKzYbdxY7AE" allowfullscreen></iframe>
    </section>

  <section>
      <h2>🧠 Explore Internal Structures</h2>
      <img src="https://upload.wikimedia.org/wikipedia/commons/e/ea/Earthworm_dissection_diagram.svg" alt="Earthworm diagram" style="width: 100%; height: auto; border: 1px solid #ccc; border-radius: 5px;">
      <ul>
        <li><strong>Pharynx:</strong> Pulls food from the mouth</li>
        <li><strong>Esophagus:</strong> Moves food to the crop</li>
        <li><strong>Crop:</strong> Temporarily stores food</li>
        <li><strong>Gizzard:</strong> Grinds food</li>
        <li><strong>Intestine:</strong> Absorbs nutrients</li>
        <li><strong>Clitellum:</strong> Reproductive segment</li>
      </ul>
    </section>

  <section>
      <h2>📝 Student Notes</h2>
      <p>Write your observations below:</p>
      <textarea rows="5" placeholder="What did you observe during the dissection?"></textarea>
    </section>

  <section>
      <h2>📄 Download Worksheet</h2>
      <p><a href="https://www.teachengineering.org/content/cub_/lessons/cub_biotech/cub_biotech_lesson01_activity2_dissectionworksheet.pdf" target="_blank">Click here to download the worksheet (PDF)</a></p>
    </section>

  <section>
      <h2>✅ Quiz</h2>
      <p><strong>Which organ in the earthworm grinds food?</strong></p>
      <ul>
        <li>a) Crop</li>
        <li><strong>b) Gizzard ✅</strong></li>
        <li>c) Intestine</li>
      </ul>
    </section>

  <section>
      <h2>📌 Summary</h2>
      <textarea rows="4" placeholder="Summarize what you learned from the earthworm dissection."></textarea>
    </section>
  </div>
</body>
</html>



